...
## 13. SESSION 2026-09-12 / 2026-09-13 — Diagnostic "0 conversion en 3 mois"

**Déclenché par** : aucune conversion depuis 3 mois malgré du budget pub, demande de diagnostic + pistes d'amélioration SEO gratuites (plus de budget pub disponible).

- **Diagnostic réel (pas de suppositions)** : Vercel Web Analytics non activé sur le projet → pivoté vers les tables Supabase directement. Résultat : 128 devis générés en 48 jours (vraies adresses variées, vrai trafic), mais **0 paiement Stripe et 0 demande de rappel réelles depuis toujours** — les 14 lignes `courses` existantes sont à 100% `source='manuel'` (saisies par l'exploitant, cash), aucune n'a de `stripe_payment_intent_id`. La fuite est entre "voir un prix" et "donner ses coordonnées/payer", pas un problème de trafic.
- **Cause racine n°1, corrigée** : l'étape 3 du widget devis mettait en avant "être rappelé sous 30 min" au lieu du paiement direct (déjà vu et corrigé plus tôt le 2026-09-12, commit `a038298`).
- **Cause racine n°2, trouvée et corrigée aujourd'hui** : le prix (widget hero ET devis) ne se calculait que si l'utilisateur cliquait une suggestion d'adresse dans le menu déroulant — taper une adresse complète et quitter le champ (Tab, clic ailleurs, clavier mobile qui masque la liste) laissait le champ non résolu, donc aucun prix, silencieusement. Fix : géocodage de secours sur `onBlur` via `/api/geocode` (commit `d61f5c0`). Vérifié en prod : adresse tapée + Tab sans clic → prix calculé quand même.
- **Bug signalé par l'utilisateur, corrigé** : clic sur le logo depuis la home elle-même ne faisait rien (Next `<Link href="/">` inerte quand la route est déjà active) → intercepté pour scroller en haut à la place (commit `ea04e0f`).
- **Nouvelle instrumentation** : table `funnel_events` (migration `20260912020000`) + helper `src/lib/funnel.ts`, posée sur chaque étape clé (widget hero, widget devis, `/reserver` avec la raison exacte des erreurs de validation). Page **`/admin/funnel`** créée pour une visibilité en libre-service (avant, il fallait me redemander à chaque fois).
- **SEO gratuit, data-driven** : script `scripts-seo-gap-analysis.mjs` (racine `owise-app/`) croise les vraies adresses recherchées (table `estimations`) avec les pages `/[destination]` existantes pour repérer la demande réelle non couverte. Résultat : **Saint-Maximin** (11 recherches réelles/48j, seulement mentionné en commune voisine) → nouvelle page dédiée `vtc-saint-maximin` créée (prix vérifié via les vraies tables `zones`/`grilles_tarifaires`, pas inventé — zone "Chantilly et environs", mêmes tarifs que Gouvieux/Chantilly).
- **Découverte incidente, statut réel non confirmé** : le SDK PostHog envoie des événements (`NEXT_PUBLIC_POSTHOG_KEY`), et une page `/admin/analytics` existante interroge PostHog via HogQL — mais `POSTHOG_PERSONAL_KEY` (clé de lecture) est absente de `.env.local`, statut sur Vercel non vérifié. Avant de s'appuyer dessus pour une future question "combien de visites" : ouvrir `/admin/analytics` en prod et voir si ça affiche de vraies données ou une erreur.
- **Tarif CDG↔Beauvais changé le 2026-09-13 vers 12:16 UTC** : 99€ → 119€ (les deux sens), à la demande directe de l'utilisateur. Mis à jour directement en base (`grilles_tarifaires`, UPDATE via service role — pas via `/admin/tarifs`, action `updatePrixGrille` non appelable hors contexte Next.js) plutôt que par migration, conformément à la clause "urgence" de la règle tarifs (documenté ici avec date et raison). Vérifié immédiatement après sur `/reserver` en prod (route dynamique, pas de cache ISR à attendre) : 119€ affiché.
- **Reset table `estimations`** : les 135 lignes historiques (dont tout le trafic d'avant les fixs ci-dessus) supprimées le **2026-09-13 vers 00:35** à la demande de l'utilisateur, pour repartir sur une base propre et mesurer l'effet réel des corrections sans le bruit des données pré-fix. `funnel_events` et `courses`/`devis` non touchées.
- Tout vérifié en conditions réelles (Puppeteer sur owise.fr en prod, pas seulement en local) avant chaque déploiement.

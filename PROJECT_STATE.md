...
- **Découverte incidente** : PostHog est déjà intégré (`POSTHOG_PERSONAL_KEY`, page `/admin/analytics` existante) — source de vraies données de trafic (visiteurs/pages vues) qui aurait évité le détour par Vercel Web Analytics si elle avait été identifiée plus tôt. À utiliser en priorité pour toute future question "combien de visites".
